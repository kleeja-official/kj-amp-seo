...
$kleeja_plugin['kj_amp_seo']['information'] = [
...
شكراً لاستخدامك إضافة كليجا هذه، قم بمراسلتنا بالأخطاء عند ظهورها على: <br>
https://github.com/kleeja-official/kj-amp-seo/issues
';

$kleeja_plugin['kj_amp_seo']['first_run']['en'] = '
Thank you for using our plugin, if you encounter any bugs and errors, visit: <br>
https://github.com/kleeja-official/kj-amp-seo/issues
';
...
// plugin functions
$kleeja_plugin['kj_amp_seo']['functions'] = [
    ...
    'b4_showsty_downlaod_id_filename' => function($args) {
        ...
        $side_menu = [
            ...
            2 => ['name' => 'fileuser', 'title' => $lang['YOUR_FILEUSER'], 'url' => $config['mod_writer'] ? 'fileuser.html' : 'ucp.php?go=fileuser', 'show' => $config['enable_userfile'] && user_can('access_fileuser')],
            3 => $user_is
                ? ['name' => 'logout', 'title' => $lang['LOGOUT'], 'url' => $config['mod_writer'] ? 'logout.html' : 'ucp.php?go=logout', 'show' => true]
                : ['name' => 'login', 'title' => $lang['LOGIN'], 'url' => $config['mod_writer'] ? 'login.html' : 'ucp.php?go=login', 'show' => true],
            4 => ['name' => 'register', 'title' => $lang['REGISTER'], 'url' => $config['mod_writer'] ? 'register.html' : 'ucp.php?go=register', 'show' => ! $user_is && $config['register']],
        ];

        $top_menu = [
            1 => ['name' => 'index', 'title' => $lang['INDEX'], 'url' => $config['siteurl'], 'show' => true],
            2 => ['name' => 'rules', 'title' => $lang['RULES'], 'url' => $config['mod_writer'] ? 'rules.html' : 'go.php?go=rules', 'show' => true],
            3 => ['name' => 'guide', 'title' => $lang['GUIDE'], 'url' => $config['mod_writer'] ? 'guide.html' : 'go.php?go=guide', 'show' => true],
            4 => ['name' => 'stats', 'title' => $lang['STATS'], 'url' => $config['mod_writer'] ? 'stats.html' : 'go.php?go=stats', 'show' => $config['allow_stat_pg'] && user_can('access_stats')],
            5 => ['name' => 'report', 'title' => $lang['REPORT'], 'url' => $config['mod_writer'] ? 'report.html' : 'go.php?go=report', 'show' => user_can('access_report')],
            6 => ['name' => 'call', 'title' => $lang['CALL'], 'url' => $config['mod_writer'] ? 'call.html' : 'go.php?go=call', 'show' => user_can('access_call')],
        ];
